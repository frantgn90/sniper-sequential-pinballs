...
TraceThread::TraceThread(Thread *thread, SubsecondTime time_start, String tracefile, String responsefile, app_id_t app_id, bool cleanup)
   ...
   , m_cleanup(cleanup)
   , m_started(false)
   , m_checkpoint_fd(open(itostr(m_thread->getAppId()).append("app.checkpoints.out").c_str(), O_CREAT | O_WRONLY | O_TRUNC, 0600))
   , last_current_time(SubsecondTime::Zero())
   , last_region_id(-1)
   , last_l1m(0)
   , last_l2m(0)
   , last_l3m(0)
   , last_instructions(0)
   , m_ignore_instructions(false)
   , m_stopped(false)
{
   ...
   m_trace.setHandleCheckpoint(TraceThread::__handleCheckpoint, this);
...
{
    uint64_t returned;
    returned = handleMagicInstruction(m_thread->getId(), a, b, c);

    if (returned == CHECKPOINT_IN_VALUE)
    {
        //std::cout << "YEAH! IN" << std::endl;
        handleCheckpointIn();
        m_ignore_instructions = true;
        return 1;
    }
    else if (returned == CHECKPOINT_OUT_VALUE)
    {
        //std::cout << "YEAH! OUT" << std::endl;
        handleCheckpointOut();
        m_ignore_instructions = false;
        return 1;
    }
...
void TraceThread::handleCheckpointIn()
{
    handleCheckpoint();
}

void TraceThread::handleCheckpointOut()
{
    last_current_time = getCurrentTime();
    last_instructions = m_thread->getCore()->getPerformanceModel()->getInstructionCount();
    last_l1m = Sim()->getStatsManager()->getMetricObject("L1-D", m_thread->getCore()->getId(), "load-misses")->recordMetric() + Sim()->getStatsManager()->getMetricObject("L1-D", m_thread->getCore()->getId(), "store-misses")->recordMetric();
    last_l2m = Sim()->getStatsManager()->getMetricObject("L2", m_thread->getCore()->getId(), "load-misses")->recordMetric() + Sim()->getStatsManager()->getMetricObject("L2", m_thread->getCore()->getId(), "store-misses")->recordMetric();
    last_l3m = Sim()->getStatsManager()->getMetricObject("L3", m_thread->getCore()->getId(), "load-misses")->recordMetric() + Sim()->getStatsManager()->getMetricObject("L3", m_thread->getCore()->getId(), "store-misses")->recordMetric();

}

bool TraceThread::handleCheckpoint()
{
    SubsecondTime current_time = getCurrentTime() - last_current_time;
    UInt64 time = current_time.getNS();
    const ComponentPeriod *dom_global = Sim()->getDvfsManager()->getGlobalDomain();
    UInt64 cycles = SubsecondTime::divideRounded(current_time, *dom_global);

    UInt64 l1_miss = Sim()->getStatsManager()->getMetricObject("L1-D", m_thread->getCore()->getId(), "load-misses")->recordMetric() + Sim()->getStatsManager()->getMetricObject("L1-D", m_thread->getCore()->getId(), "store-misses")->recordMetric() - last_l1m;
    UInt64 l2_miss = Sim()->getStatsManager()->getMetricObject("L2", m_thread->getCore()->getId(), "load-misses")->recordMetric() + Sim()->getStatsManager()->getMetricObject("L2", m_thread->getCore()->getId(), "store-misses")->recordMetric() - last_l2m;
    UInt64 l3_miss = Sim()->getStatsManager()->getMetricObject("L3", m_thread->getCore()->getId(), "load-misses")->recordMetric() + Sim()->getStatsManager()->getMetricObject("L3", m_thread->getCore()->getId(), "store-misses")->recordMetric() - last_l3m;

    UInt64 instructions = m_thread->getCore()->getPerformanceModel()->getInstructionCount() - last_instructions;
    ...
    line <<       m_app_id
        ...
        << "|" << time << "|" << cycles
        << "|" << instructions
        << "|" << l1_miss << "|" << l2_miss << "|" << l3_miss << std::endl;

    write(m_checkpoint_fd, (void *)line.str().c_str(), line.tellp());
...
void TraceThread::handleInstructionDetailed(Sift::Instruction &inst, Sift::Instruction &next_inst, PerformanceModel *prfmdl)
{
    ...
    if (m_ignore_instructions)
    {
        //std::cout << "Ignoring instruction." << std::endl;
        return;
    }
