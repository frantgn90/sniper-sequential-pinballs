...
class TraceThread : public Runnable
{
   private:
      ...
      static bool __handleCheckpoint(void* arg)
      { return ((TraceThread*)arg)->handleCheckpoint(); }
      ...
      // checkpoint
      SubsecondTime last_current_time; //for time and cycles
      int64_t last_region_id;
      uint64_t last_l1m;
      uint64_t last_l2m;
      uint64_t last_l3m;
      uint64_t last_instructions;

      bool m_ignore_instructions;


      void handleCheckpointIn();
      void handleCheckpointOut();
